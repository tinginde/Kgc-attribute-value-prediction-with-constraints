...
from Data_Processing_nonorm import *
...
def main():
    ...
    for epoch in range(epochs):
        ...
        if np.mean(loss_record['att_h_val']) < best_mse: 
            ...
            torch.save(model.state_dict(),'exp/saved_model/nonorm_ast_model_{}_{}_{}.pt'.format(epochs, batch_size,learning_rate))
            

        #with open('exp/loss_record/ast_model_{}_{}_{}.pickle'.format(epochs, batch_size,learning_rate),'wb') as fw:
    ...
    save_result(table, 'exp/predicted_result/nonorm_ast_epoch{}_batch{}_preds_att_head.csv'.format(epochs,batch_size)) 
