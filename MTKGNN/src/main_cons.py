...
def main():
    ...
    for epoch in range(epochs):
        ...
        for x_batch_head_attr, y_batch_head_attr in train_loader_head_attr:
            ...
            if gdp in x[:,1]:
                tri_idx = [i for i, idx in enumerate(x[:,1].tolist()) if idx ==25 ]
                x_constraint = torch.tensor([num_lit[x[tri_idx,0].cpu().numpy()][i][pop_idx]*num_lit[x[tri_idx,0].cpu().numpy()][i][gdp_per] for i in range(len(tri_idx))])
                x_constraint = x_constraint.to(device)          
                loss_2 = model.cal_loss(output2, torch.reshape(y.float(), (-1,1))) + model.cal_loss(output2[tri_idx],x_constraint.unsqueeze(1)) 
            else:
            ...
            #     output2 = model.AttrNet_h_forward(x[:,0], x[:,1])
            #     loss_2 = model.cal_loss(output2, torch.reshape(y.float(), (-1,1))) + model.cal_loss(output2,y_criterion) 
            #     loss_2.backward()
            #     optimizer.step()
            #     loss_record['att_h_train'].append(loss_2.detach().cpu().item())
            # else:
        ...
        if np.mean(loss_record['att_h_val']) < best_mse: 
            ...
            torch.save(model.state_dict(),'exp_cons/saved_model/vargdp_model_{}_{}_{}_128_64.pt'.format(epochs, batch_size,learning_rate))
    ...
    save_result(table, 'exp_cons/predicted_result/vargap_{}_{}_128_64preds_att_head.csv'.format(epochs,batch_size)) 
