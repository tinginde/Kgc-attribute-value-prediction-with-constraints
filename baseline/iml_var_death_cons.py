...
config = {
    ...
    'save_path': './baseline/iml_pt/model_{}_cons_ver2.pt'.format(var_predict) , # your model will be saved here
...
#             nn.Dropout(0.5),
    ...
    def cal_loss(self, preds, targets, x):
        ...
        for prediction, target in zip(preds, targets):
            ...
            mask = (prediction < x[:,date_of_birth]+70).float() 
...
import pickle
with open('baseline/iml_cons/loss_record_{}_cons_ver2.pickle'.format(var_predict),'wb') as fw:
...
#print(preds)
save_pred(preds, 'baseline/iml_cons/pred_result_{}_cons_ver2'.format(var_predict))
