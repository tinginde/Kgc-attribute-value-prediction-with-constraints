...
def train(tr_set, dv_set, model, config, device):
    ...
    for epoch in range(n_epochs):
        ...
        if dev_mse < min_mse:
            ...
            #torch.save(model.state_dict(), config['save_path']+'model_2layer_e500.pt')  # Save model to specified path
            early_stop_cnt = 0
        else:
            early_stop_cnt += 1
...
# with open(config['save_path']+'model_2layer_e500.pickle','wb') as fw:
...
#del model
model = NeuralNet(328).to(device)
ckpt = torch.load('models_3layer/model_3layer_attless.pt', map_location='cpu')  # Load your best model
model.load_state_dict(ckpt)
#plot_pred(dv_set, model, device)  # Show prediction on the validation set


preds = test(valid_loader, model, device)  # predict 
save_pred(preds, config['save_path']+'preds_result_model_3layer_attmore')       # save prediction file to pred.csv
