...
# X_trainset, y_trainset= train_attri_data[['e','a','new_stdv']], train_attri_data['v']
# X_validset, y_validset= valid_attri_data[['e','a','new_stdv']], valid_attri_data['v']  
# X_testset, y_testset= test_attri_data[['e','a','new_stdv']], test_attri_data['v']  
...
class KGMTL_Data(Dataset):
    ...
    # cat[emb_e, emb_a]: input; ['v']:target
    def __init__(self, attri_data):
        attri_data, y_set = attri_data[['e','a']], attri_data[['new_stdv']] 

        self.y= torch.FloatTensor(y_set.to_numpy())
        ...
        self.x = torch.cat([entity_embedding,attribute_embedding],dim=1)
...
train_set =KGMTL_Data(train_attri_data)
valid_set =KGMTL_Data(valid_attri_data)
test_set =KGMTL_Data(test_attri_data)
...
def train(tr_set, dv_set, model, config, device):
    ...
    for epoch in range(n_epochs):
        ...
        if dev_mse < min_mse:
            ...
            torch.save(model.state_dict(), config['save_path']+'model_new.pt')  # Save model to specified path
            early_stop_cnt = 0
        else:
            early_stop_cnt += 1
...
with open('models_ver2/linear_new.pickle','wb') as fw:
...
save_pred(preds, 'models_ver2/preds_result_new')       # save prediction file to pred.csv
