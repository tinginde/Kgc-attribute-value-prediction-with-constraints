...
config = {
    ...
    'save_path': 'baseline/iml_cons_realv_pt/model_{}_cons_mean.pt'.format(var_predict),
...
def inver_trans(idx):
    min_x_cons = attri_data[attri_data['a']==id].v.min()
    max_x_cons = attri_data[attri_data['a']==id].v.max()
    return min_x_cons, max_x_cons

min_pop_cons,max_pop_cons=inver_trans('P1082')
min_gdpper_cons, max_gdpper_cons = inver_trans('P2299')
...
#             nn.Dropout(0.5),
    ...
    def cal_loss(self, preds, targets, x):
        ...
        for prediction, target in zip(preds, targets):
            ...
            x_pop = x[:,pop_idx]*3.466321e+07+ 1.013151e+06
            x_gdp_per=x[:,gdp_per]*24895.688674+ 20586.915589
            #mask = (prediction != 1.013151e+06 * 20586).float()  
            # Give a punishment of 1000 to the predicted values that are greater than 2.5
            punished_prediction =1.013151e+06 * x_gdp_per
            # Calculate the loss between the punished prediction and the target
            loss = (prediction - punished_prediction).pow(2).mean()
            losses.append(loss)
...
import pickle
with open('baseline/iml_cons_realv/loss_record_{}_cons_mean.pickle'.format(var_predict),'wb') as fw:
...
#print(preds)
save_pred(preds, 'baseline/iml_cons_realv/pred_result_{}_cons_mean'.format(var_predict))
