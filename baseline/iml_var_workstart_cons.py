...
def make_inputdata(var, num_lit,num_lit_valid,target,target_val):
    ...
    x_constraint = np.array([target[e][var_cons] for e in list(ent)])

    x_train = df_train[cols]
    x_valid = df_valid[cols]

    return x_train, y_train, x_valid, y_valid, x_constraint

x_train, y_train, x_valid, y_valid, x_cons_workend = make_inputdata(var_predict, num_lit_minmax, num_lit_minmax_valid,num_lit,num_lit_valid)
...
#attdf = attri_data[attri_data['a']=='P2032']
#attdf['ori']= v_scale(attri_data_train[attri_data_train['a']=='P2032']).inverse_transform(x[:,work_end].cpu().detach().numpy().reshape(-1,1))
min_cons = attri_data[attri_data['a']=='P2032'].v.min()
max_cons = attri_data[attri_data['a']=='P2032'].v.max()
...
config = {
    ...
    'save_path': './baseline/iml_cons_realv_pt/model_{}_cons.pt'.format(var_predict) , # your model will be saved here
...
#             nn.Dropout(0.5),
    ...
    def cal_loss(self, preds, targets,x):
        ''' Calculate loss '''
        # TODO: you may implement L1/L2 regularization here
        losses = []
        for prediction, target in zip(preds, targets):
            # Find the predicted values that are greater than workend
            #workend need to back to  realvalue
            x_constraint = x[:,work_end]*29+ 1992
            mask = (prediction >x_constraint).float() 
            # Give a punishment of 1000 to the predicted values that are greater than workend
...
import pickle
with open('baseline/iml_cons_realv/loss_record_{}_cons.pickle'.format(var_predict),'wb') as fw:
...
#print(preds)
save_pred(preds, 'baseline/iml_cons_realv/pred_result_{}_cons'.format(var_predict))
