...
os.makedirs('models_layer_tanh/', exist_ok=True)  # The trained model will be saved to ./models/
...
config = {
    ...
    'save_path': 'models_layer_tanh/' , # your model will be saved here
}
...
def train(tr_set, dv_set, model, config, device):
    ...
    for epoch in range(n_epochs):
        ...
        if dev_mse < min_mse:
            ...
            torch.save(model.state_dict(), config['save_path']+'model_1layer_tanh_e500.pt')  # Save model to specified path
            early_stop_cnt = 0
        else:
            early_stop_cnt += 1
...
with open(config['save_path']+'model_1layer_tanh_e500.pickle','wb') as fw:
...
ckpt = torch.load(config['save_path']+'model_1layer_tanh_e500.pt', map_location='cpu')  # Load your best model
...
save_pred(preds, config['save_path']+'preds_result_model_1layer_tanh_e500')       # save prediction file to pred.csv
