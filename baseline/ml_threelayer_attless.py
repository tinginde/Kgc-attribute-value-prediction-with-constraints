...
def train(tr_set, dv_set, model, config, device):
    ...
    for epoch in range(n_epochs):
        ...
        if dev_mse < min_mse:
            ...
            torch.save(model.state_dict(), config['save_path']+'model_3layer_attless.pt')  # Save model to specified path
            early_stop_cnt = 0
        else:
            early_stop_cnt += 1
...
model = NeuralNet(328).to(device)  # Construct model and move to device
print(model)
"""# **Start Training!**"""
import pickle
...
with open(config['save_path']+'model_3layer_attless.pickle','wb') as fw:
...
ckpt = torch.load(config['save_path']+'model_3layer_attless.pt', map_location='cpu')  # Load your best model
...
save_pred(preds, config['save_path']+'preds_result_model_3layer_attless')       # save prediction file to pred.csv
