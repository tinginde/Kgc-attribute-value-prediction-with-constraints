...
train_attri_data=train_attri_data[['e','a','v']]
valid_attri_data=valid_attri_data[['e','a','v']]
test_attri_data=test_attri_data[['e','a','v']]
## constraint needed:
...
y_trainset, y_validset,y_testset = train_attri_data.loc[:,'v'].to_numpy(),valid_attri_data.loc[:,'v'].to_numpy(),test_attri_data.loc[:,'v'].to_numpy()
...
os.makedirs('models_minmax_1229v/', exist_ok=True)  # The trained model will be saved to ./models/
...
config = {
    ...
    'save_path': 'models_minmax_1229v/' , # your model will be saved here
}
...
def train(tr_set, dv_set, model, config, device):
    ...
    for epoch in range(n_epochs):
        ...
        if dev_mse < min_mse:
            ...
            torch.save(model.state_dict(), config['save_path']+'model_minmax_1layer_b200_v.pt')  # Save model to specified path
            early_stop_cnt = 0
        else:
            early_stop_cnt += 1
...
with open(config['save_path']+'model_minmax_1layer_b200_v.pickle','wb') as fw:
...
ckpt = torch.load(config['save_path']+'model_minmax_1layer_b200_v.pt', map_location='cpu')  # Load your best model
...
save_pred(preds, config['save_path']+'preds_result_model_minmax_1layer_v200_v')     # save prediction file to pred.csv
